...
foreach ($pp in $parameterPatterns) {
    ...
    $functionName = "TestByRef_function_$($pp.Pat)"
...
try {
    $resultList.Succeeded = ' + "$($functionName) " + $str5a + '$parsedDateTime' + $str5b + '
} catch {
...
Remove-Variable -Name resultList -Force'
    ...
    $fnc | Out-File -FilePath (Join-Path (Split-Path $PSScriptRoot -Parent) "function\$($functionName).ps1") -Encoding UTF8
}